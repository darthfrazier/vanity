...
func registerHandlers() {
	...
	// TODO:: implement
	// r.Methods("GET").Path("/pieces").Handler(appHandler(getPiecesHandler))
	// r.Methods("GET").Path("/outfit").Handler(appHandler(getOutfitsHandler))
...
func updateOutfitHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	err = vanity.DB.UpdateOutfit(&piece)
	if err != nil {
		return appErrorf(err, "could not save outfit: %v", err)
	}
	return nil
